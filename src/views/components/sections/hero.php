...
function renderBreadcrumb($breadcrumb)
{
    ...
    $html = '<nav class="mb-6" aria-label="Breadcrumb"><ol class="flex items-center space-x-2 text-sm text-white">';
    foreach ($breadcrumb as $i => $item) {
        ...
        if (!empty($item['href'])) {
            ...
            $html .= '<li class="font-semibold text-white">' . htmlspecialchars($item['label']) . '</li>';
        }
    }
    $html .= '</ol></nav>';
    return $html;
...
<section id="hero" class="relative overflow-hidden bg-gradient-to-br from-primary/90 to-primary2/80 py-20 md:py-32 text-white">
    <div class="container mx-auto px-4 flex flex-col lg:flex-row items-center gap-12">
        <div class="flex-1 flex flex-col justify-center z-10">
            ...
            <h1 class="text-4xl md:text-5xl lg:text-5xl font-extrabold leading-tight mb-6 drop-shadow-xl" data-animate="fade-in-up">
                <?= htmlspecialchars_decode($heroTitle) ?>
            </h1>
            <p class="text-lg md:text-xl text-white/80 mb-8 max-w-2xl drop-shadow-lg" data-animate="fade-in-up">
